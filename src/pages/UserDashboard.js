...
const Dashboard = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        const filteredAppointments = res.data.filter((app) => app.userId === selectedUserId); 
      ...
      axios.get('http://localhost:5000/users').then((res) => {
        const filteroption = res.data.filter((app) => app.id === selectedUserId);
        console.dir('users :', filteroption);
